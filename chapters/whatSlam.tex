...
\begin{Russian}
	...
	\textbf{Цели лекции}
	\begin{enumerate}[labelindent=0em,leftmargin=1.5em]
        \item Понять, из каких модулей состоит визуальный SLAM и какую задачу выполняет каждый из модулей.
        \item Настроить среду программирования и подготовиться к экспериментам.
        \item Понять, как скомпилировать и запустить программу под Linux, и как ее отлаживать, в случае возникновения проблем.
        \item Изучить основы использования CMake.
	\end{enumerate}
\end{mdframed}
\end{Russian}
...
\begin{Russian}
    Эта лекция дает общее представление о структуре системы визуального SLAM.
    Практическая часть лекции знакомит с основами по настройке среды разработки программ и самой разработкой. 
    Завершится лекция написанием небольшой программы: ``Hello SLAM''.
\end{Russian}
...
\begin{Russian}
    \section{Встречайте ``Little Carrot''}
    Предположим, мы собрали робота по имени \emph{Little Carrot}, который продемонстрирован на следующем рисунке:
\end{Russian}
...
\begin{Russian}
    \begin{figure}
    	\centering
    	\includegraphics[width=0.8\textwidth]{resources/whatIsSLAM/carrot.pdf}
    	\caption{Эскиз робота \emph{Little Carrot}}
    \end{figure}
\end{Russian}
...
\begin{Russian}
    Хотя робот немного похож на робота Android, он не имеет ничего общего с системой Android. В его багажник мы кладем ноутбук (чтобы в любой момент можно было отлаживать программы). Итак, что мы ожидаем от робота?

    Мы рассчитываем, что \textit{Little Carrot} умеет двигаться автономно. Многие бытовые роботы статично размещены на рабочих столах. Они могут общаться с людьми или проигрывать музыку, но современные планшеты также могут выполнять те же задачи. Мы надеемся, что \textit{Little Carrot} сможет свободно передвигаться по комнате. Где бы мы ни поздоровались с ним, он может сразу к нам приехать.
\end{Russian}
...
\begin{Russian}
    Во-первых, такому роботу нужны колеса и моторы для движения, поэтому мы установили на него колеса (управление походкой для роботов-гуманоидов очень сложное, рассматривать их не будем). Теперь с колесами робот может двигаться, но без эффективной навигационной системы \textit{Little Carrot} не знает, где находится цель действия, и ничего не может делать, кроме как бродить вслепую. Хуже того, он может удариться о стену и нанести ущерб. Чтобы этого избежать, мы установили ему на голову камеры, интуитивно понимая, что такой робот должен быть похож на человека. Действительно, с глазами, мозгом и конечностями люди могут свободно ходить и исследовать окружающую среду, поэтому мы (как-то наивно) думаем, что наш робот тоже должен быть в состоянии это делать. Что ж, чтобы \textit{Little Carrot} мог исследовать комнату, мы обнаружили, что ему необходимо знать как минимум две вещи:
    \begin{enumerate}
    	\item Где я? - Речь идет о \emph{локализации}.
        \item Что из себя представляет окружающая среда? - Речь идет о \emph{создании карты}.
    \end{enumerate}
\end{Russian}
...
\begin{Russian}
    \textit{Локализацию} и \textit{создание карты} можно рассматривать как внутренне и внешнее восприятие. Будучи полностью автономным роботом, \textit{Little Carrot} должен понимать свое собственное состояние (т.е. местоположение) и внешнюю среду (т.е. \ карту). Конечно, существует множество различных подходов к решению этих двух проблем. Например, мы можем положить на пол комнаты направляющие, или наклеить на стену множество искусственных маркеров, таких как QR-коды, или установить на столе устройства радиолокации. Если мы находимся на улице, мы также можем установить GNSS приемник (например, на подобии того, что есть в мобильном телефоне или автомобиле) на голове \textit{Little Carrot}. Можем ли мы утверждать, что с этими устройствами проблема локализации решена? Давайте разделим эти датчики (см. рис.~\ref{fig:sensors}) на два класса.
\end{Russian}
...
\begin{Russian}
    \begin{figure}
    	\centering
    	\includegraphics[width=0.8\textwidth]{./resources/whatIsSLAM/sensors.jpg}
    	\caption{Различные виды датчиков: (а) QR-код; (b) приемник ГНСС;  (с) направляющие; (d) лазерный дальномер; (e) инерциальная единица измерения; (f) стерео-камера;}
    	\label{fig:sensors}
    \end{figure}
\end{Russian}
...
\begin{Russian}
    Первый класс — это неинтрузивные датчики, которые полностью автономны внутри робота, такие как колесные энкодеры, камеры, лазерные сканеры и т. д. Они не предполагают модификации среды вокруг робота для работы с ними. Другой класс — это интрузивные датчики, требующие подготовки среды, такие как вышеупомянутые направляющие, QR-коды и т. д. Интрузивные датчики обычно могут напрямую определить местонахождение робота, решая проблему локализации простым и эффективным способом. Однако, поскольку они требуют изменений в среде, область их использования часто в определенной степени ограничена. Например, если нет сигнала GPS или нельзя проложить направляющие, что делать в этих случаях?
\end{Russian}
...
\begin{Russian}
    Мы видим, что интрузивные датчики накладывают определенные ограничения на внешнюю среду. Система локализации, основанная на них, может функционировать должным образом только тогда, когда эти ограничения соблюдаются в реальном мире. В противном случае подход к локализации не может быть реализован, так, например, система GPS локализации в помещении работает плохо или не работает вовсе. Поэтому, хотя этот тип датчика надежен и прост, он не работает как универсальное решение. Напротив, неинтрузивные датчики, такие как лазерные сканеры, камеры, колесные энкодеры, инерциальные измерительные блоки (IMU) и т. д., могут измерять только косвенные физические величины, а не прямое местоположение. Например, колесный энкодер измеряет угол поворота колеса. IMU измеряет угловую скорость и ускорение. Камера или лазерный сканер получают данные наблюдения за внешней средой в определенной форме, такой как облака точек и изображения. В этом случае, чтобы определить позицию робота из этих косвенных наблюдений, мы должны применить алгоритмы. Хотя это звучит как обходная тактика, более очевидным преимуществом является то, что такой подход не предъявляет никаких требований к среде, что позволяет применять подход для локализации в неизвестной среде. Поэтому во многих областях исследований подход называют самолокализацией.
\end{Russian}
...
\begin{Russian}
    Оглядываясь назад на определение SLAM, обсуждавшееся ранее, мы подчеркивали неизвестную среду в задачах SLAM. Мы не накладываем ограничения на то, в какой среде будет использоваться \textit{Little Carrot} (но на самом деле у нас будет приблизительный диапазон, например, в помещении или на улице). Это значит, что мы не можем предполагать, что внешние датчики, такие как GPS будут работать бесперебойно. Поэтому использование портативных неинтрузивных датчиков для решения задачи SLAM является нашим основным направлением. В частности, говоря о визуальном SLAM, мы имеем в виду использование \textit{камер} для решения проблем локализации и построения карты.
\end{Russian}
...
\begin{Russian}
    Визуальный SLAM — основная тема этой книги, поэтому нас особенно интересует, на что способны глаза \textit{Little Carrot}. Камеры, используемые в SLAM, отличаются от широко известных зеркальных камер (SLR --- single-lens reflex) камер. Нам подойдет камера, которая намного дешевле и не используют дорогой объектив, но  формирует непрерывный видеопоток. Обычная камера может снимать изображения со скоростью 30 кадров в секунду, а высокоскоростные камеры могут делать это быстрее. Камеры можно условно разделить на три категории: монокулярные, стерео и RGB-D, как показано на следующем рисунке~\ref{fig:cameras}. Интуитивно понятно, что монокулярная камера имеет только одну камеру. У стерео-камеры их две. Принцип действия камеры RGB-D сложнее. Помимо получения цветных изображений, она также может измерять расстояние сцены от камеры для каждого пикселя изображения. Устройства RGB-D обычно имеют несколько камер и могут использовать различные принципы работы. В пятой лекции мы подробно расскажем об их принципах работы, а пока читателям хватит и интуитивного понимания. Некоторые новые специальные типы камер также могут быть применены в SLAM, например, панорамная камера~\cite{Pretto2011}, камера событий~\cite{Rueckauer2016}. Иногда их можно увидеть в SLAM приложениях, но они пока не стали массовыми. Судя по внешнему виду \textit{Little Carrot}, можем сделать вывод, что у нашего робота есть стерео-камера.
\end{Russian}
...
\begin{Russian}
    \begin{figure}
    	\centering
    	\includegraphics[width=0.8\textwidth]{./resources/whatIsSLAM/camera.pdf}
    	\caption{Различные типы камер: монокулярная, RGB-D, стерео.}
    	\label{fig:cameras}
    \end{figure}
    Теперь давайте взглянем на плюсы и минусы использования различных типов камер для SLAM.
\end{Russian}
...
\begin{Russian}
    \subsubsection{Монокулярная камера}

    Система SLAM, в которой используется только одна камера, называется монокуляным SLAM. В виду простого набора сенсоров и из низкой стоимости монокулярный SLAM оказался очень привлекательным для исследователей. Вы наверняка видели выходные данные монокулярной камеры - это фото. Что мы можем сделать с одной-единственной фотографией?
\end{Russian}
...
\begin{Russian}
    Фотография — это, по сути, \textit{проекция} сцены на плоскость изображения камеры. Она отражает трехмерный мир в двухмерном виде. Очевидно, что в процессе проецирования теряется одно измерение: так называемая \textit{глубина} (или расстояние). В случае монокулярной камеры, используя одно изображение, мы не можем получить расстояние между объектами в сцене и камерой. Позже мы увидим, что знание глубины критично для SLAM. Поскольку мы, люди, видели много фотографий, у нас сформировалось естественное чувство расстояния для большинства сцен, что помогло нам определить расстояние между объектами на изображении. Например, мы можем распознавать объекты на изображении и соотносить их с их приблизительными размерами, полученными из повседневного опыта. Близкие объекты будут загораживать удаленные объекты; солнце, луна и другие небесные объекты бесконечно далеки; объект будет иметь тень, если на него падает солнечный свет. Этот здравый смысл может помочь нам определить расстояние до объектов, но есть также определенные случаи, которые сбивают нас с толку, в результате мы не можем определить расстояние и надлежащий размер объекта. На рисунке~\ref{fig:why-depth} показан такой пример. По этому изображения мы не сможем определить, являются ли фигурки настоящими людьми или маленькими игрушками, до тех пор пока мы не изменим угол обзора и не исследуем трехмерную структуру сцены. объект может быть большой, но далекий, а может быть близкий, но маленький. В то же время, на изображении такие объекты могут казаться одного размера из-за эффекта перспективной проекции.
\end{Russian}
...
\begin{Russian}
    \begin{figure}
    	\centering
    	\includegraphics[width=0.8\textwidth]{./resources/whatIsSLAM/why-depth.pdf}
    	\caption{По одному изображению мы не можем сказать, являются ли эти люди настоящими или это просто маленькие игрушки.}
    	\label{fig:why-depth}
    \end{figure}
\end{Russian}
...
\begin{Russian}
    Поскольку изображение, полученное монокулярной камерой, представляет собой всего лишь двухмерную проекцию трехмерного пространства, то для восстановления трехмерной структуры, мы должны изменить угол обзора камеры. Монокуляреый SLAM использует тот же принцип. Перемещая камеру, мы оцениваем ее движение, а также расстояния до объектов на сцене и их размеры. Итак, как мы это делаем? Из повседневного опыта мы знаем, что если камера движется вправо, объекты на изображении будут двигаться влево, что позволяет нам делать выводы о движении. С другой стороны, мы также знаем, что близкие к камере объекты движутся быстрее, а удаленные — медленнее. Таким образом, при движении камеры движение этих объектов на изображении формирует \textit{disparity} --- пиксельное несоответствие. Вычисляя несоответствие, мы можем количественно определить, какие объекты находятся далеко, а какие близко.
\end{Russian}
...
\begin{Russian}
    Однако даже если мы знаем, какие объекты находятся близко, а какие далеко, это все равно лишь относительные величины. Например, когда мы смотрим фильм, мы можем сказать, какие объекты на сцене больше, чем другие, но мы не можем определить реальный размер этих объектов — являются ли здания настоящими высотными зданиями или просто макетами на столе? Это настоящий монстр, который разрушает здание, или просто актер в специальной одежде? Интуитивно понятно, что если движение камеры и размер сцены удваиваются одновременно, монокулярные камеры видят то же самое. Точно так же, умножая этот размер на любой коэффициент, мы все равно получим ту же картину. Это говорит о том, что траектория и карта, полученные в результате монокулярного SLAM, будут отличаться от фактической траектории и карты с неизвестным масштабным коэффициентом \textit{scale} \footnote{Математическая причина этого явления будет объяснена в главе, посвященной визуальной одометрии.}. Поскольку монокулярный SLAM не позволяет определить реальный масштаб исключительно на основе изображений, это называют неоднозначностью масштаба.

     В монокулярном SLAM глубина может быть рассчитана только при поступательном движении, а реальный масштаб не может быть определен. Эти две вещи могут вызвать серьезные проблемы при применении монокулярного SLAM в реальных приложениях. Основная причина в том, что глубину нельзя определить по одному изображению. Чтобы получить реальную глубину, нужно использовать стерео и RGB-D камеры.
\end{Russian}
...
\begin{Russian}
    \subsubsection{Стерео-камеры и RGB-D камеры}
     Целью использования стерео-камер и RGB-D камер является измерение расстояния между объектами и камерой. Таким образом преодолеваются недостатки монокулярных камер, заключающиеся в том, что расстояния неизвестны. Как только расстояния становятся известны, трехмерная структура сцены может быть восстановлена по одному кадру, а неоднозначность масштаба устраняется. Хотя и стереокамеры, и камеры RGB-D могут измерять расстояние, принципы их работы различаются. Стереокамера состоит из двух синхронизированных монокулярных камер, смещенных на известное расстояние друг от друга. Такое расстояние называют базовой линией. Поскольку величина базовой линии известна, мы можем рассчитать трехмерное положение каждого пикселя так же, как это делают наши глаза. Человек оценивает расстояния до объектов, основываясь на различиях между изображениями для левого и правого глаза, и мы можем попытаться сделать то же самое на компьютерах (см. рис.~\ref{fig:stereo}). Мы также можем расширить стерео-камеру до многокамерной системы, если это необходимо, но в принципе особой разницы нет.
\end{Russian}
...
\begin{Russian}
    \begin{figure}
        \centering
        \includegraphics[width=0.8\textwidth]{./resources/whatIsSLAM/stereo.pdf}
        \caption{Расстояние рассчитывается по несоответствию двух пар стерео-изображений.}
        \label{fig:stereo}
    \end{figure}
\end{Russian}
...
\begin{Russian}
    Стерео-камеры обычно требуют значительных вычислительных ресурсов для (качественной) оценки глубины каждого пикселя, что выглядит необычно, если сравнивать с тем, как это делает человек. Диапазон глубин, измеренный стерео-камерой, связан с длиной базовой линии. Чем длиннее базовая линия, тем дальше может быть измерена глубина. Поэтому стерео-камеры, устанавливаемые на автономные транспортные средства, обычно довольно большие. Оценка глубины для стерео-камер достигается путем сравнения изображений с левой и правой камер без использования другого сенсорного оборудования. Таким образом, стерео-камеры могут применяться как внутри помещений, так и снаружи. Недостатком стерео-камер или многокамерных систем является сложность процесса настройки и калибровки. Их диапазон глубины и точность ограничены длиной базовой линии и разрешением камеры. Кроме того, стерео-сопоставление и вычисление диспаратности (различий между изображениями) также потребляют много вычислительных ресурсов и обычно требуют использования GPU или FPGA для создания карт глубины в режиме реального времени. Таким образом, в большинстве современных алгоритмов требовательность к вычислительным ресурсам по-прежнему является одной из основных проблем стерео-камер.
\end{Russian}
